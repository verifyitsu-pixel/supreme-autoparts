...
add_action('template_redirect',function(){$path=trim((string)parse_url($_SERVER['REQUEST_URI']??'',PHP_URL_PATH),'/');$map=['account/shipping-payment-policy'=>'shipping-payment-policy','account/warranty-and-return'=>'warranty-and-return','account/privacy-policy'=>'privacy-policy','account/cookie-policy'=>'privacy-policy','account/track-order'=>'track-order','account/submit-ticket'=>'submit-ticket','account/testimonials'=>'testimonials','account/payment-details'=>'checkout','account/faq'=>'faq','blog/blog-listing'=>'blog'];$key=strtolower($path);if(isset($map[$key])){wp_safe_redirect(home_url('/'.$map[$key].'/'),301);exit;}});
